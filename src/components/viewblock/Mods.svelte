<script lang="ts">
    ...
    async function refreshDownloadsLive() {
        ...
        try {
            ...
            if (
                ...
                const items = await invoke<ArchiveModItem[]>(
                    "fetch_repo_mods",
                    {
                        sort,
                    },
    ...
    $effect(() => {
        if (
            $catalogResetNonce === 0 ||
            $catalogResetNonce === lastCatalogReset
        ) {
            return;
        }
    ...
    onMount(() => {
        ...
        return () => {
            ...
            try {
                if (typeof unlistenModsChanged === "function")
                    unlistenModsChanged();
            } catch (err) {
    ...
    async function updateAllMods(e?: Event) {
        ...
        try {
            ...
            const previousEnabledMap: Record<string, boolean> =
                Object.fromEntries(
                    await Promise.all(
        ...
        modsStore.update((arr) =>
            arr.map((m) => {
                ...
                const hasThumb = image
                    ? !/\/images\/cover\.jpg$/i.test(image)
                    : false;
                const updated: Mod = { ...m };
                if (
                    (!updated.description ||
                        updated.description.trim().length === 0) &&
                    desc
                ) {
                ...
                if (
                    image &&
                    (!updated.image ||
                        /\/images\/cover\.jpg$/i.test(updated.image))
                ) {
                    updated.image = image;
        ...
        try {
            ...
            try {
                const thumbItems = items
                    .filter(
                        (i) => i.image_url && /^https?:\/\//i.test(i.image_url),
                    )
    ...
    async function fillDescriptions(mods: (Mod & { _dirName?: string })[]) {
        ...
        async function worker() {
            while (true) {
                ...
                if (
                    !m ||
                    hasMeaningfulDescription(m.description ?? "", m.title)
                )
                    continue;
    ...
    async function fillDescriptionsVisibleFirst() {
        ...
        await Promise.all(
            new Array(Math.min(limit, candidates.length))
                .fill(0)
                .map(() => worker()),
        );
        applyBatch();
    ...
    async function fillCachedDescriptions(mods: Mod[]) {
        ...
        async function worker() {
            while (true) {
                ...
                if (idx >= mods.length) break;
                const m = mods[idx];
                if (!m || (m.description && m.description.trim().length > 0))
                    continue;
                try {
    ...
    function mergeIncomingMods(incomingMods: Mod[]): number {
        ...
        modsStore.update((arr) => {
            ...
            const pruneAllowed =
                incoming.size > 0 &&
                incoming.size >=
                    Math.max(10, Math.floor(existingRemoteCount * 0.5));
            ...
            for (const title of incomingOrder) {
                ...
                if (existing) {
                    ...
                    out.push({
                        ...
                        ...inc,
                        requires_steamodded:
                            existing.requires_steamodded ||
                            inc.requires_steamodded,
                        requires_talisman:
    ...
    let selectedCollection = $derived(
        selectedCollectionId
            ? ($collectionsStore.find((c) => c.id === selectedCollectionId) ??
                  null)
            : null,
    );
    ...
    async function handleShareCollection(id: string) {
        ...
        if (!result.ok || !result.code) {
            addMessage(
                result.error || "Failed to generate share code.",
                "error",
            );
            return;
        }
